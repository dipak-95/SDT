// import { motion } from "framer-motion";
// import { FaEnvelope } from "react-icons/fa";
// import {
//   FaFacebookF,
//   FaInstagram,
//   FaWhatsapp,

// } from "react-icons/fa6";
// import { Link } from "react-router-dom";

// export default function Footer() {
//   const whatsappMessage = encodeURIComponent(
//     `Hello, I am interested in your tours. Please share details.`
//   );
//   const socialLinks = [
//     {
//       icon: FaFacebookF,
//       url: "https://www.facebook.com/share/16em1TNgUn/",
//       label: "Facebook"
//     },
//     {
//       icon: FaInstagram,
//       url: "https://www.instagram.com/saurashtradarshantours?utm_source=qr&igsh=MWQxaWJ3MDgzanV5Zg==",
//       label: "Instagram"
//     },
//     {
//       icon: FaEnvelope,
//       url: "mailto:saurashtradarshantour@gmail.com",
//       label: "Email"
//     }
//   ];
//   const companyLinks = [
//   { label: "How To Book", path: "/tours/group" },
//   { label: "Hotel Booking", path: "/hotels" },
//   { label: "Car Rental", path: "/rentalcar" },
//   { label: "Payment Options", path: "/payment-options" },
//   { label: "See Our Journy", path: "/pastjournies" }
// ];

// const supportLinks = [
//   { label: "Contact Us", path: "/contact" },
//   { label: "Guest Review", path: "/pastjournies" },
//   { label: "Tour Booking", path: "/tours/group" },
//   { label: "Booking Policy", path: "/booking-policy" },
//   { label: "Refund Policy", path: "/refund-policy" }
// ];

// const destinationLinks = [
//   { label: "Dwarka Tour Packages", path: "/tours/group" },
//   { label: "Somnath Tour Packages", path: "/tours/group" },
//   { label: "Gir Tour Packages", path: "/tours/group" },
//   { label: "Kutch Tour Packages", path: "/tours/group" },
//   { label: "Vadodara Tour Packages", path: "/tours/group" }
// ];

// const packageLinks = [
//   { label: "Dwarka Somnath Tour", path: "/tours/group" },
//   { label: "Gir Jungle Safari Tour", path: "/tours/group" },
//   { label: "Dwarka Somnath Gir & Diu Tour", path: "/tours/group" },
//   { label: "Statue Of Unity Tour", path: "/tours/group" },
//   { label: "Gujarat Desert Beach Tour", path: "/tours/group" }
// ];


//   return (
//     <footer className="bg-gradient-to-b from-[#071c1f] to-[#020d10] text-white pt-16">

//       {/* ================= TOP FOOTER ================= */}
//       <div className="max-w-7xl mx-auto px-6 py-2">
//   <div
//     className="
//       flex
//       flex-col
//       gap-10
//       sm:flex-row
//       sm:flex-wrap
//       lg:flex-nowrap
//       lg:justify-between
//     "
//   >
//     {/* Company */}
//     <div className="w-full sm:w-[45%] lg:w-[22%]">
//       <h4 className="text-lg font-semibold mb-4">Company</h4>
//       <ul className="space-y-2 text-sm text-gray-300">
//         {companyLinks.map((item, i) => (
//           <li key={i}>
//             <Link
//               to={item.path}
//               className="hover:text-[#F4612B] transition"
//             >
//               {item.label}
//             </Link>
//           </li>
//         ))}
//       </ul>
//     </div>

//     {/* Support */}
//     <div className="w-full sm:w-[45%] lg:w-[22%]">
//       <h4 className="text-lg font-semibold mb-4">Support Us</h4>
//       <ul className="space-y-2 text-sm text-gray-300">
//         {supportLinks.map((item, i) => (
//           <li key={i}>
//             <Link
//               to={item.path}
//               className="hover:text-[#F4612B] transition"
//             >
//               {item.label}
//             </Link>
//           </li>
//         ))}
//       </ul>
//     </div>

//     {/* Trending Destinations */}
//     <div className="w-full sm:w-[45%] lg:w-[22%]">
//       <h4 className="text-lg font-semibold mb-4">
//         Trending Destinations
//       </h4>
//       <ul className="space-y-2 text-sm text-gray-300">
//         {destinationLinks.map((item, i) => (
//           <li key={i}>
//             <Link
//               to={item.path}
//               className="hover:text-[#F4612B] transition"
//             >
//               {item.label}
//             </Link>
//           </li>
//         ))}
//       </ul>
//     </div>

//     {/* Trending Packages */}
//     <div className="w-full sm:w-[45%] lg:w-[22%]">
//       <h4 className="text-lg font-semibold mb-4">
//         Trending Packages
//       </h4>
//       <ul className="space-y-2 text-sm text-gray-300">
//         {packageLinks.map((item, i) => (
//           <li key={i}>
//             <Link
//               to={item.path}
//               className="hover:text-[#F4612B] transition"
//             >
//               {item.label}
//             </Link>
//           </li>
//         ))}
//       </ul>
//     </div>
//   </div>
// </div>



//       {/* ================= BOTTOM FOOTER ================= */}
//       <div className="bg-white text-gray-800 mt-16 rounded-t-3xl">
//         <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 md:grid-cols-3 gap-8 items-center">

//           {/* Logo & About */}
//           <div>
//             <img src="/logo.webp" alt="Logo" className="h-12 mb-3" />
//             <p className="text-sm text-gray-600">
//               Travel with Gujarat experts to experience seamless journeys,
//               authentic destinations and unforgettable memories.
//             </p>
//           </div>

//           {/* Contact */}
//           <div>
//             <h4 className="font-semibold mb-2">Connect with Different Support</h4>
//             <p className="text-sm text-gray-600">
//               Near tower chowk, New Patelwada, Veraval, Gujarat 362265, India
//             </p>
//             <p className="text-sm mt-1">📞 +91 9979922797</p>
//             <p className="text-sm">✉️ saurashtradarshantour@gmail.com</p>
//           </div>

//           {/* Social */}
//           <div>
//             <h4 className="font-semibold mb-3">Social Media</h4>

//             <div className="flex gap-3 text-lg">
//               {socialLinks.map(({ icon: Icon, url, label }, i) => (
//                 <motion.a
//                   key={i}
//                   href={url}
//                   target={label !== "Email" ? "_blank" : undefined}
//                   rel="noopener noreferrer"
//                   whileHover={{ scale: 1.15 }}
//                   className="
//           w-10 h-10
//           rounded-full
//           bg-[#F4612B]
//           text-white
//           flex
//           items-center
//           justify-center
//           cursor-pointer
//         "
//                   aria-label={label}
//                 >
//                   <Icon />
//                 </motion.a>
//               ))}
//             </div>
//           </div>

//         </div>

//         {/* Copyright */}
//         {/* <div className="text-center text-sm py-4 border-t">
//           © {new Date().getFullYear()} Gujarat Expert. All Rights Reserved.
//         </div> */}
//       </div>

//       {/* ================= WHATSAPP FLOAT ================= */}
//       <a
//         href={`https://wa.me/919979922797?text=${whatsappMessage}`}
//         target="_blank"
//         rel="noopener noreferrer"
//         className="fixed bottom-6 left-6 bg-green-500 text-white px-4 py-3 rounded-full shadow-lg flex items-center gap-2 z-50"
//       >
//         <FaWhatsapp className="text-xl" />
//         <span className="text-sm font-semibold hidden sm:block">
//           Discuss On WhatsApp
//         </span>
//       </a>

//     </footer>
//   );
// }
import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import { motion, useScroll, useTransform } from "framer-motion";
import {
  FaFacebookF,
  FaInstagram,
  FaWhatsapp,
  FaEnvelope,
  FaArrowUp,
} from "react-icons/fa6";
import { ScrollToTopButton } from "./ScrollToTopButton";

export default function Footer() {
  

  /* ================= DATA ================= */
  const whatsappMessage = encodeURIComponent(
    "Hello, I am interested in your tours. Please share details."
  );

  const socialLinks = [
    { icon: FaFacebookF, url: "https://facebook.com" },
    { icon: FaInstagram, url: "https://instagram.com" },
    { icon: FaEnvelope, url: "mailto:saurashtradarshantour@gmail.com" },
  ];

  const companyLinks = [
    { label: "How To Book", path: "/tours/group" },
    { label: "Hotel Booking", path: "/hotels" },
    { label: "Car Rental", path: "/rentalcar" },
    { label: "Payment Options", path: "/payment-options" },
    { label: "Our Journey", path: "/pastjournies" },
  ];

  const supportLinks = [
    { label: "Contact Us", path: "/contact" },
    { label: "Guest Review", path: "/pastjournies" },
    { label: "Booking Policy", path: "/booking-policy" },
    { label: "Refund Policy", path: "/refund-policy" },
  ];

  const destinationLinks = [
    { label: "Dwarka Tour", path: "/tours/group" },
    { label: "Somnath Tour", path: "/tours/group" },
    { label: "Gir Tour", path: "/tours/group" },
    { label: "Kutch Tour", path: "/tours/group" },
    { label: "Vadodara Tour", path: "/tours/group" },
  ];

  const packageLinks = [
    { label: "Dwarka Somnath Tour", path: "/tours/group" },
    { label: "Gir Safari Tour", path: "/tours/group" },
    { label: "Somnath Gir Diu Tour", path: "/tours/group" },
    { label: "Statue of Unity Tour", path: "/tours/group" },
  ];

  return (
    <footer className="bg-gradient-to-b from-[#071c1f] to-[#020d10] text-white pt-16 relative">

      {/* ================= TOP FOOTER ================= */}
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col gap-10 sm:flex-row sm:flex-wrap lg:flex-nowrap lg:justify-between">
          {[companyLinks, supportLinks, destinationLinks, packageLinks].map(
            (links, idx) => (
              <div key={idx} className="w-full sm:w-[45%] lg:w-[22%]">
                <h4 className="text-lg font-semibold mb-4">
                  {["Company", "Support", "Destinations", "Packages"][idx]}
                </h4>
                <ul className="space-y-2 text-sm text-gray-300">
                  {links.map((item, i) => (
                    <li key={i}>
                      <Link
                        to={item.path}
                        className="hover:text-[#F4612B] transition"
                      >
                        {item.label}
                      </Link>
                    </li>
                  ))}
                </ul>
              </div>
            )
          )}
        </div>
      </div>

      {/* ================= BOTTOM ================= */}
      <div className="bg-white text-gray-800 mt-16 rounded-t-3xl">
        <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 md:grid-cols-3 gap-8 items-center">

          <div>
            <img src="/logo.webp" alt="Logo" className="h-12 mb-3" />
            <p className="text-sm text-gray-600">
              Travel with Gujarat experts for authentic and seamless journeys.
            </p>
          </div>

          <div>
            <h4 className="font-semibold mb-2">Contact</h4>
            <p className="text-sm text-gray-600">
              Veraval, Gujarat 362265, India
            </p>
            <p className="text-sm mt-1">📞 +91 9979922797</p>
            <p className="text-sm">✉️ saurashtradarshantour@gmail.com</p>
          </div>

          <div>
            <h4 className="font-semibold mb-3">Social Media</h4>
            <div className="flex gap-3">
              {socialLinks.map(({ icon: Icon, url }, i) => (
                <motion.a
                  key={i}
                  href={url}
                  target="_blank"
                  rel="noopener noreferrer"
                  whileHover={{ scale: 1.15 }}
                  className="w-10 h-10 rounded-full bg-[#F4612B] text-white flex items-center justify-center"
                >
                  <Icon />
                </motion.a>
              ))}
            </div>
          </div>
        </div>
      </div>

      {/* ================= SCROLL TO TOP ================= */}
     <ScrollToTopButton />
      {/* ================= WHATSAPP ================= */}
      <motion.a
        href={`https://wa.me/919979922797?text=${whatsappMessage}`}
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 left-6 z-50 bg-green-500 text-white px-4 py-3 rounded-full shadow-lg overflow-hidden flex items-center gap-2"
        whileHover={{ scale: 1.08 }}
      >
        {/* GLASS SHINE */}
        <motion.div
          className="absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent skew-x-12"
          initial={{ x: "-120%" }}
          animate={{ x: "120%" }}
          transition={{ repeat: Infinity, duration: 2.5, ease: "easeInOut" }}
        />
        <FaWhatsapp className="text-xl relative z-10" />
        <span className="hidden sm:block text-sm font-semibold relative z-10">
          Discuss on WhatsApp
        </span>
      </motion.a>
    </footer>
  );
}
